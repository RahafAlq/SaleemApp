...
import sa.ksu.gpa.saleem.stepsCounterUtils.SensorFilter.dot
import sa.ksu.gpa.saleem.stepsCounterUtils.SensorFilter.norm
import sa.ksu.gpa.saleem.stepsCounterUtils.SensorFilter.sum
...
class StepDetector {
    ...
    fun updateAccel(
        ...
        worldZ[0] = sum(accelRingX) / Math.min(
            accelRingCounter,
            ACCEL_RING_SIZE
        )
        worldZ[1] = sum(accelRingY) / Math.min(
            accelRingCounter,
            ACCEL_RING_SIZE
        )
        worldZ[2] = sum(accelRingZ) / Math.min(
            accelRingCounter,
            ACCEL_RING_SIZE
        )
        val normalization_factor = norm(worldZ)
        worldZ[0] = worldZ[0] / normalization_factor
        worldZ[1] = worldZ[1] / normalization_factor
        worldZ[2] = worldZ[2] / normalization_factor
        val currentZ = dot(worldZ, currentAccel) - normalization_factor
        velRingCounter++
